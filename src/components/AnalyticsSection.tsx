...
export function AnalyticsSection({
    ...
    return (
      <section className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Analytics aren't available yet — they'll appear once listening activity is recorded.
        </p>
      </section>
    );
  }
  ...
  const cards = [
    {
      label: scope === "listener" ? "Minutes listened" : "Minutes streamed",
      value: formatMinutes(data.minutesListened),
      icon: Clock3,
    },
    {
      label: scope === "listener" ? "Plays" : "Streams",
      value: data.streams.toLocaleString(),
      icon: Headphones,
    },
    {
      label: scope === "listener" ? "Tracks explored" : "Tracks",
      value: data.uniqueSongs.toLocaleString(),
      icon: Music2,
    },
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Last 30 days · updated when a track starts playing
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-4">
          ...
          <div className="h-32 flex items-end gap-1.5">
            {data.daily.map((day) => (
              <div
                ...
                title={`${day.date}: ${day.streams} plays`}
              >
                <div
                  className="w-full rounded-t bg-primary/70 group-hover:bg-primary transition-colors"
                  style={{ height: `${Math.max(day.streams ? 8 : 2, (day.streams / max) * 100)}%` }}
                />
              </div>
            ))}
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          ...
          {data.topTracks.length === 0 ? (
            <p className="text-sm text-muted-foreground">No listening activity yet.</p>
          ) : (
